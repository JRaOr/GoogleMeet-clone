...
export default function RoomComponent( { room, participants, track, user, showChat, dominantSpeaker} ) {
    ...
    return(
        <div className={`participants-grid flex flex-col md:grid overflow-scroll py-10 md:py-0 hide-scroll gap-2 mx-auto ${people < 7 ? 'max-w-[1050px]':'max-w-[1545px]'} relative ${people < 3 ? `participants-${people}`: people < 7 ? 'participants-3' : 'participants-4'}`}>
            ...
            {participants.length > 0 ? <div className='rounded-md overflow-hidden bg-[#36373a] flex items-center justify-center w-full relative max-w-[504px] h-[283px] my-auto mx-auto'>
                {_videoTrack()}
